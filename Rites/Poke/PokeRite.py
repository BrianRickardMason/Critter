...
class PokeRite(Rite):
    ...
    def __init__(self, aCritterData, aSettings, aPostOffice):
        ...
        """
        Rite.__init__(self, aCritterData, aSettings, aPostOffice, Rites.RiteCommon.POKE, PokeMessageProcessor)

    def run(self):
        """Starts the main loop of the rite."""
        while True:
            self.mLogger.debug("Poking other critters.")
            envelope = self.mPostOffice.encode("PokeAnnouncement", {'critterData': self.mCritterData})
